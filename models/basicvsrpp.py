...
import warnings
from basicvsr import SpyNet, ConvResidualBlocks


class BasicVSRPlusPlus(keras.models.Model):
    ...
    def __init__(self,
        ...
        if is_low_res_input:
            ...
            self.feat_extract = nn.Sequential(
                layers.Conv2D(mid_channels, 3, 2, padding='same', activation=layers.LeakyReLU(.1)),
                layers.Conv2D(mid_channels, 3, 2, padding='same', activaion=layers.LeakyReLU(.1)),
        ...
        self.upconv1 = layers.Conv2D(mid_channels * 4, 3, 1, padding='same', bias=True)
        self.upconv2 = layers.Conv2D(64 * 4, 3, 1, padding='same', bias=True)

        self.pixel_shuffle = nn.PixelShuffle(2)

        self.conv_hr = layers.Conv2D(64, 3, 1, padding='same')
        self.conv_last = layers.Conv2D(3, 3, 1, padding='same')
        self.img_upsample = layers.UpSamling2D(size=4, interpolation='bilinear')
